                        ...
                        <CardHeader className="bg-primary/5">
                            <div className="flex items-center justify-between">
                                ...
                                <div className="flex flex-col items-end gap-2">
                                    <Badge className={cn("text-sm px-4 py-2", getStatusColor(order.status))}>
                                        {order.status_label}
                                    </Badge>
                                    <Badge className={cn("text-sm px-4 py-2", getPaymentStatusColor(order.payment_status))}>
                        ...
                        <Card className="border">
                            ...
                            <CardContent>
                                <div className="space-y-4">
                                    <div className="flex items-center justify-between">
                                        <div>
                                            <p className="text-sm text-muted-foreground">{__('Progress')}</p>
                                            <p className="text-3xl font-black">{metrics.percentage}%</p>
                                        </div>
                                        <div className="text-right">
                                            <p className="text-sm text-muted-foreground">{__('Steps Completed')}</p>
                                    ...
                                    {order.shipping_steps.map((step, index) => (
                                        <div key={step.id} className="relative pl-12">
                                            {index !== order.shipping_steps.length - 1 && (
                                                <div className={cn(
                                                    "absolute left-[19px] top-10 bottom-[-24px] w-1 rounded-full z-0",
                                                    step.is_reached ? "bg-primary" : "bg-secondary"
                                                )} />
                                            )}

                                            <div className={cn(
                                                "absolute left-0 top-2 size-10 rounded-full border-4 border-background flex items-center justify-center z-10 shadow-sm",
                                                step.is_reached ? "bg-primary text-primary-foreground" : "bg-secondary text-secondary-foreground"
                                            )}>
                                                {step.is_reached ? (
                                                    <CheckCircle2 size={18} />
                                                ) : (
                                                    <span className="text-xs font-bold">{step.position}</span>
                                                )}
                                            </div>
                            ...
                            <CardContent>
                                <div className="space-y-2">
                                    ...
                                    {order.shipping_address.city && (
                                        <p className="ml-6">{order.shipping_address.city}</p>
                                    )}
